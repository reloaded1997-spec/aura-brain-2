...
const STATUS_STYLES = {
  active:   { bg: 'bg-[#E8F0E9] dark:bg-[#1B2620]', text: 'text-[#4A7A54] dark:text-[#80A789]', label: 'Active' },
  achieved: { bg: 'bg-[#F5EDD8] dark:bg-[#2A231A]', text: 'text-[#8A6A35] dark:text-[#C49A6C]', label: 'Achieved' },
  archived: { bg: 'bg-[#EEEAE2] dark:bg-[#272320]', text: 'text-[#8A8580] dark:text-[#827C70]', label: 'Archived' },
};
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#FAF8F3] dark:bg-[#171511] text-[#26241F] dark:text-[#ECE7DD]">
      ...
      <main className="flex-1 px-4">
        ...
        <div className="mb-4 flex gap-1 rounded-xl bg-[#EFEAE0] dark:bg-[#24221C] p-1">
          {[
        ...
        {pageTab === 'habits' && (
          <>
              ...
              <div className="mt-6">
                ...
                <div className="overflow-hidden rounded-[18px] border border-[#EBE6DC] dark:border-[#322E27] bg-white dark:bg-[#221F1B]">
                  {habits.map((h) => {
                    ...
                    return (
                      <div key={h.id} className="flex items-center border-b border-[#F1ECE2] dark:border-[#272320] last:border-b-0">
                        <div className="flex min-w-0 flex-1 items-center gap-3 px-4 py-3">
                          <span className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-[10px] bg-[#EFEADF] dark:bg-[#2A2620] font-['Newsreader'] text-[15px] text-[#6F6A60] dark:text-[#A39C8E]">
                            {initialOf(h.title)}
                          </span>
                          <span className="min-w-0 flex-1">
                            <span className="block font-['Newsreader'] text-[15px] text-[#26241F] dark:text-[#ECE7DD]">{h.title}</span>
                            <span className="block text-[12px] text-[#9A958A] dark:text-[#827C70]">{habitSub(h)}</span>
                            {parentGoal && (
                              <span className="mt-0.5 flex items-center gap-1 text-[11px] text-[#A8845C] dark:text-[#C49A6C]">
            ...
            {habits.length === 0 && (
              <div className="mt-8 text-center font-['Newsreader'] text-[15px] italic text-[#B6B0A2] dark:text-[#6A645A]">
          ...
          <>
            ...
            {sortedGoals.length > 0 && (
              <div className="mt-6 flex flex-col gap-3">
                <div className="mx-1 text-[10px] font-semibold uppercase tracking-[1.8px] text-[#9A958A] dark:text-[#827C70]">
                  Your goals
                </div>
                {sortedGoals.map((goal) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full rounded-[18px] border border-[#EBE6DC] dark:border-[#322E27] bg-white dark:bg-[#221F1B] p-4 text-left shadow-[0_1px_2px_rgba(40,36,31,0.03)] dark:shadow-[0_1px_2px_rgba(0,0,0,0.32)] transition-colors hover:bg-[#FAF6EF] dark:hover:bg-[#272320]"
                    >
                      {/* Title row */}
                      <div className="flex items-start justify-between gap-2">
                        <span className="font-['Newsreader'] text-[17px] leading-snug text-[#1F1D18] dark:text-[#F1EDE5]">
                          {goal.title}
                        </span>
                        <span
                      ...
                      {/* Description */}
                      {goal.description ? (
                        <p className="mt-1 font-['Newsreader'] text-[13px] italic text-[#9A958A] dark:text-[#827C70]">
                          {goal.description}
                        </p>
                      ) : null}

                      {/* Meta row */}
                      <div className="mt-2 flex items-center gap-3 text-[12px] text-[#A8A294] dark:text-[#6E685D]">
                        {dateLabel && (
